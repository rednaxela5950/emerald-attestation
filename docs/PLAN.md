...
- [x] 0.2 Initialize tooling:
  - Foundry or Hardhat for contracts.
  - Node + TypeScript toolchain (pnpm or npm).
- [x] 0.3 Add basic `README.md` with project summary and how to run tests.
- [x] 0.4 Commit initial scaffold (`chore: bootstrap project`).
...
- [x] 1.1 Design `EmeraldPostRegistry` (or `EmeraldBoardDemo`):
...
- [x] 1.2 Design `EmeraldDaAdapter` interface:
  - Storage for:
...
- [x] 1.3 Create `MockKzgVerifier` contract:
...
- [x] 1.4 Write Foundry tests (or equivalent) for:
...
- [x] 1.5 Commit with message like `feat: add core Emerald DA contracts (Phase1 shell)`.
...
- [x] 3.1 Define challenge storage:
...
- [x] 3.3 Implement `finalizePostFromCustody(postId)`:
...
- [x] 3.5 Commit with message like `feat: add custody challenge logic with mock KZG verifier`.
